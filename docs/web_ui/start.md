...
![Web UI](https://iggy.apache.org/img/iggy_web_ui.png)

These instructions target Iggy server 0.9.0. Use a matching source checkout for local builds.
...
When building from source, generate the static assets before compiling the server. Run from the repository root:
...
The feature alone does not generate these assets. A server binary built without them returns 404 at `/ui`.
...
Start an Iggy server with HTTP enabled, as shown in [Getting started](https://iggy.apache.org/docs/introduction/getting-started), then run the Web UI as a separate container:

```bash
docker pull apache/iggy-web-ui:0.4.0
docker run -e PUBLIC_IGGY_API_URL=http://localhost:3000 -p 3050:3050 apache/iggy-web-ui:0.4.0
```
...
The Web UI lives in the `web/` folder of the [repository](https://github.com/apache/iggy/tree/master/web). Use a Node.js version supported by the frontend toolchain (`^20.19.0 || ^22.13.0 || >=24`, for example Node.js 22.13 or later in the 22.x series) and `npm` (`pnpm` and `yarn` are not part of the supported workflow):
...
- **Topics** - create and delete topics, and edit their name and message expiry
...
- **Messages** - browse messages per partition with built-in decoders:
  ...
  - XML decoder
- **Users** - list and create users, including their initial permissions
- **Settings** - Web UI preferences

Topic updates share the HTTP API's [retention update limitations](https://iggy.apache.org/docs/server/topic-options). Other topic options require the CLI or HTTP API.

The Clients, Logs and terminal routes are placeholders hidden from navigation. The Server settings page displays information, but saving settings is disabled. Editing and deleting existing users, changing their permissions and bulk user actions are also placeholders. See the [roadmap](https://github.com/apache/iggy/tree/master/web#roadmap) for what is planned.

## Docker Compose example

Here's the full example of the `docker-compose.yml` file that starts the Iggy server, initializes it with the `my-stream` stream and `my-topic` topic with the help of [Iggy CLI](https://iggy.apache.org/docs/cli/start), and starts the Iggy Web UI:
